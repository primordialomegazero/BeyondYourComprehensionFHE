...
class TrueFractalZKP {
public:
    static constexpr double PHI = 1.6180339887498948482;
    static constexpr size_t FRACTAL_DEPTH = 7;
    ...
    static SchnorrProof prove(const std::string& data) {
        ...
        BIGNUM* c = BN_new(); BN_hex2bn(&c, sha256(proof.commitment_R + "||" + proof.public_key_Y).c_str()); BN_mod(c, c, n, ctx);
    ...
    static bool verify(const SchnorrProof& proof) {
        EC_GROUP* g = EC_GROUP_new_by_curve_name(NID_secp256k1);
        BN_CTX* ctx = BN_CTX_new();
        const BIGNUM* n = EC_GROUP_get0_order(g);
        EC_POINT* R = EC_POINT_new(g); EC_POINT* Y = EC_POINT_new(g);
        hex_to_point(g, R, proof.commitment_R); hex_to_point(g, Y, proof.public_key_Y);
        BIGNUM* cp = BN_new(); BN_hex2bn(&cp, sha256(proof.commitment_R + "||" + proof.public_key_Y).c_str()); BN_mod(cp, cp, n, ctx);
        BIGNUM* c = BN_new(); BN_hex2bn(&c, proof.challenge_c.c_str());
        if(BN_cmp(cp, c) != 0) { BN_free(cp); BN_free(c); EC_POINT_free(R); EC_POINT_free(Y); EC_GROUP_free(g); BN_CTX_free(ctx); return false; }
        BIGNUM* s = BN_new(); BN_hex2bn(&s, proof.response_s.c_str());
    ...
    static std::vector<SchnorrProof> recursive_fractal_prove(const std::string& data, size_t d=FRACTAL_DEPTH) {
